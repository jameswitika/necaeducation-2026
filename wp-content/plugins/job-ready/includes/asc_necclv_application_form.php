...
// Short Course (NECCLV001) Application Form (NECCLV001)
add_filter("gform_pre_render_" . SHORT_COURSE_APPLICATION_NECCLV001, 'short_course_accredited_necclv_form_prepopulate');
add_filter("gform_pre_validation_" . SHORT_COURSE_APPLICATION_NECCLV001, 'short_course_accredited_necclv_form_prepopulate');
add_filter("gform_pre_submission_filter_" . SHORT_COURSE_APPLICATION_NECCLV001, 'short_course_accredited_necclv_form_prepopulate');
add_filter("gform_admin_pre_render" . SHORT_COURSE_APPLICATION_NECCLV001, 'short_course_accredited_necclv_form_prepopulate');

// Short Course (NECCLV004) Application Form (NECCLV004)
add_filter("gform_pre_render_" . SHORT_COURSE_APPLICATION_NECCLV004, 'short_course_accredited_necclv_form_prepopulate');
add_filter("gform_pre_validation_" . SHORT_COURSE_APPLICATION_NECCLV004, 'short_course_accredited_necclv_form_prepopulate');
add_filter("gform_pre_submission_filter_" . SHORT_COURSE_APPLICATION_NECCLV004, 'short_course_accredited_necclv_form_prepopulate');
add_filter("gform_admin_pre_render" . SHORT_COURSE_APPLICATION_NECCLV004, 'short_course_accredited_necclv_form_prepopulate');

function short_course_accredited_necclv_form_prepopulate($form)
{
	$prefill_fields= array();
	
	if(ASC_NECCLV_DEBUG_MODE)
	{
		echo "Prefill: <br/>";
	...
	if(isset($_SESSION['prefill']))
	{
		...
		*/
		...
		$prefill_fields['137'] = $_SESSION['prefill']->street_number;
		$prefill_fields['138'] = $_SESSION['prefill']->street_name;
		...
		$prefill_fields['139'] = $_SESSION['prefill']->postal_street_number;
		$prefill_fields['140'] = $_SESSION['prefill']->postal_street_name;
		...
		if($employee_party_xml_object!== false)
		{
			...
			if($_SESSION['employee_party_id'] == $employee_party_xml_object->{'party'}->{'id'})
			{
				...
				*/
				...
				$prefill_fields['137'] = $form_fields->street_number;
				$prefill_fields['138'] = $form_fields->street_name;
				...
				$prefill_fields['139'] = $form_fields->postal_street_number;
				$prefill_fields['140'] = $form_fields->postal_street_name;
...
function short_course_accredited_necclv_form_presubmission()
{
	...
	$prefill->street_number = $_POST['input_137'];
	$prefill->street_name = $_POST['input_138'];
	...
	$prefill->postal_street_number = $_POST['input_139'];
	$prefill->postal_street_name = $_POST['input_140'];
...
// NECCLV001 Form Process
add_action ( 'gform_after_submission_' . SHORT_COURSE_APPLICATION_NECCLV001, 'short_course_necclv_form_accredited_submission_process', 10, 2 );
// NECCLV004 Form Process
add_action ( 'gform_after_submission_' . SHORT_COURSE_APPLICATION_NECCLV004, 'short_course_necclv_form_accredited_submission_process', 10, 2 );

function short_course_necclv_form_accredited_submission_process($entry, $form_data) 
{
	if (ASC_NECCLV_DEBUG_MODE) 
	{
		echo "<h3>Short Course NECCLV001 or NECCLV004 Form - Submission Process</h3><br/><br/>";
		error_log ( "<h3>Short Course NECCLV001 or NECCLV004 Form - Submission Process</h3><br/><br/>" );
		
// 		echo "<h3>Entry Field: </h3><br/>";
// 		var_dump ( $entry );
// 		echo "<br/><br/>";
		
		// echo "<h3>Form Data Field: </h3><br/>";
		// var_dump($form_data);
		// echo "<br/><br/>";
	}
	...
	$form->street_number = ucwords ( strtolower ( rgar ( $entry, '137' ) ) );
	$form->street_name = ucwords ( strtolower ( rgar ( $entry, '138' ) ) );
	...
	if ($form->postal_address_same != 'Yes') 
	{
		...
		$form->postal_street_number = ucwords ( strtolower ( rgar ( $entry, '139' ) ) );
		$form->postal_street_name = ucwords ( strtolower ( rgar ( $entry, '140' ) ) );
	...
	$form->emergency_contact_email = rgar ( $entry, '121' );
	...
	$form->australian_citizen = rgar ( $entry, '30' );
	
	if ($form->australian_citizen) 
	{
		$form->citizenship_status = rgar ( $entry, '77' );
	} 
	else
	{
		$form->citizenship_other = rgar ( $entry, '31' );
	}
	...
	$form->year_highest_school_level = rgar ( $entry, '49' );
	...
	if (strpos ( $form->previous_victorian_education, 'Yes' ) !== false && rgar ( $entry, '58' ) != '') 
	{
		$form->previous_victorian_school = rgar ( $entry, '58' );
	}
	elseif (strpos ( $form->previous_victorian_education, 'Yes' ) !== false && rgar ( $entry, '59' ) != '') 
	{
		$form->previous_victorian_school = rgar ( $entry, '59' );
	}
	...
	// Create PDF
	$aafp = short_course_necclv_application_form_pdf ( $form );
	...
	*/
	...
	if (isset ( $party_id )) 
	{
		...
		if (isset ( $party_document_result->id )) 
		{
			...
			// Remove PDF from server (if not testing)
			if(!ASC_NECCLV_DEBUG_MODE)
			{
				unlink ( $pdf_file );
			}
			...
			if (ASC_NECCLV_DEBUG_MODE) 
			{
				...
				echo "GF Keep Array: <br/>";
				var_dump ( $gf_keep_array );
				echo "<br/><br/>";
				...
				echo "GF Party ID Field: ";
				var_dump ( $gf_party_id_field );
				echo "<br/><br/>";
			}
...
// ******************************************************************* //
// SHORT COURSE APPLICATION FORM (NECCLV001 + NECCLV004) PROCESS - END //
// ******************************************************************* //
...
// Setup Short Course (NECCLV004) Application Form PDF
function short_course_necclv_application_form_pdf($form) 
{
	// Start
	$content = neca_pdf_content_start ();
	
	// Add Heading
	$content .= neca_pdf_content_heading ( 'Short Course Accredited ' . $form->course_number . ' Form', $form );
	...
	// Office Use Only
	$content .= neca_pdf_content_office_use_only_accredited_short_course ( $form, $show_dates = false );
