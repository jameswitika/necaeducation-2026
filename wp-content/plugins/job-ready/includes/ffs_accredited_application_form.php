...
// FFS Accredited Application Form (ASC)
...
function short_course_accredited_application_form_prepopulate($form)
{
	...
	{
		// 2024.01.04 - Add new field
		$prefill_fields['142'] = $_SESSION['prefill']->previously_enrolled_at_neca;
		...
		*/
		...
		$prefill_fields['148'] = $_SESSION['prefill']->labour_force_status;
		$prefill_fields['87'] = $_SESSION['prefill']->country_of_birth;
		$prefill_fields['145'] = $_SESSION['prefill']->indigenous_status;
		...
		$prefill_fields['151'] = $_SESSION['prefill']->highest_school_level;
		...
		$prefill_fields['157'] = $_SESSION['prefill']->rpl;
		...
		// 04.01.2024 - Add Concession Card - Requested by Lyn Wang
		$prefill_fields['158'] = isset($_SESSION['prefill']->concession_flag) ? $_SESSION['prefill']->concession_flag : '';
		...
		if($employee_party_xml_object!== false)
		{
			...
			if($_SESSION['employee_party_id'] == $employee_party_xml_object->{'party'}->{'id'})
			{
				...
				*/
				...
				$prefill_fields['148'] = $form_fields->labour_force_status;
				$prefill_fields['87'] = $form_fields->country_of_birth;
				$prefill_fields['145'] = $form_fields->indigenous_status;
				...
				$prefill_fields['151'] = $form_fields->highest_school_level;
...
function short_course_accredited_application_form_presubmission()
{
	...
	$prefill->previously_enrolled_at_neca = $_POST['input_142'];
	...
	// 04.01.2024 - Concession Card Added - Requested by Lyn Wang
	$prefill->concession_flag = $_POST['input_158'];
...
function short_course_application_form_accredited_submission_process($form_data, $order, $item_cost) 
{
	if (ASC_DEBUG_MODE) 
	{
		echo "<h3>FFS Accredited Application Form - Submission Process</h3>";
	}
	...
	$form->previously_enrolled_at_neca = $form_data->{'142'};
	...
	// Labour Force
	$form->labour_force_status = $form_data->{'148'};
	
	// Referred from a Job Seeker
	$form->referred = $form_data->{'146'};
	$form->referred_details = $form_data->{'147'};
	
	// Birth + Nationality + Indigenous + Language
	$form->country_of_birth = $form_data->{'87'};
	$form->indigenous_status = $form_data->{'145'};
	...
	$form->highest_school_level = $form_data->{'151'};
	...
	// 2024.01.04 - Credit Transfer and RPL seperated and requested to be added by Lyn Wang
	$form->credit_transfer = $form_data->{'132'};
	$form->rpl = $form_data->{'157'};
	
	// 2024.01.04 - Added File Field for USI Transcript
	$form->file_usi_transcript = $form_data->{'155'};	
	...
	// 04.01.2024 - Added Concession Flag - Requested by Lyn Wang
	$form->concession_flag = $form_data->{'158'};
	...
	if(ASC_DEBUG_MODE)
	{
		echo '<a href="' . JR_ROOT_URL . '/pdf/' . $aafp. '" target="_blank">FFS Accredited Application Form (PDF)</a><br/><br/>';
	}
	...
	*/
	...
	if(isset ( $party_id ))
	{
		...
		// 2024.01.04 - Upload the USI TRANSCRIPT FILE Party Document
...
// Setup FFS Accredited Application Form PDF
function short_course_accredited_application_form_pdf($form) 
{
	...
	// 04.01.2024 - Added Credit Transfer - Requested by Lyn Wang
	$content .= neca_pdf_content_credit_transfer ( $form );
	
	// Recognition of Prior Learning
	// 04.01.2024 - Added Recognition of Prior Learning - Requested by Lyn Wang
	...
	// 04.01.2024 - Added Concession Card - Requested by Lyn Wang
...
function neca_pdf_content_office_use_only_asc( $form, $show_dates = true ) 
{
	...
	$content .= '<table class="tbl">
					<tr>
						<td colspan="2"><strong>PROGRAM DETAILS (Accredited)</strong></td>
					</tr>
					<tr>
						<tr>
							<td style="width: 5%;">[&nbsp;&nbsp;&nbsp;]</td>
