<?php
function neca_display_next_course_date_shortcode($atts) {
	...
	if (count($job_ready_dates) > 0) {
		foreach ($job_ready_dates as $job_ready_date) {
			if (!$job_ready_date->meta->full) {
				...
				return sprintf(
						'%s to %s - see below for more dates',
...
add_shortcode('neca_next_course_date', 'neca_display_next_course_date_shortcode');



function neca_display_course_dates_shortcode($atts) {
...
// Register the shortcode
add_shortcode('neca_course_dates_table', 'neca_display_course_dates_shortcode');