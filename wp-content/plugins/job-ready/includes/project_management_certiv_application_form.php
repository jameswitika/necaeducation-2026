...
add_filter("gform_pre_render_" . PROJECT_MANAGEMENT_CERTIV_APPLICATION_FORM, 'project_management_certiv_form_prepopulate');
add_filter("gform_pre_validation_" . PROJECT_MANAGEMENT_CERTIV_APPLICATION_FORM, 'project_management_certiv_form_prepopulate');
add_filter("gform_pre_submission_filter_" . PROJECT_MANAGEMENT_CERTIV_APPLICATION_FORM, 'project_management_certiv_form_prepopulate');
add_filter("gform_admin_pre_render" . PROJECT_MANAGEMENT_CERTIV_APPLICATION_FORM, 'project_management_certiv_form_prepopulate');

function project_management_certiv_form_prepopulate($form)
{
	$prefill_fields= array();
	
	if(PROJECT_MGMNT_CERTIV_DEBUG_MODE)
	{
 ...
 * *************** */
// Project Management Form (PM-APP)
add_action( "gform_pre_submission_" . PROJECT_MANAGEMENT_CERTIV_APPLICATION_FORM, 'project_management_certiv_form_presubmission');

function project_management_certiv_form_presubmission()
{
	if(PROJECT_MGMNT_CERTIV_DEBUG_MODE)
	{
 ...
 * @return boolean
 */
function project_management_certiv_form_submission_process($entry, $form_data) 
{
	if (PROJECT_MGMNT_CERTIV_DEBUG_MODE) 
	{
	...
	// Citizenship Add again as per form provided 26/01/2022
	$form->australian_citizen = $entry ['187'];
	if ($form->australian_citizen == 'Yes') 
	{
		$form->citizenship_status = 'Australian Citizen';
	}
	else 
	{
...
// 	$form->occupation = rgar ( $entry, '66' );
	...
	$pmfp = project_management_certiv_application_form_pdf ( $form );
...
add_action ( 'gform_after_submission_' . PROJECT_MANAGEMENT_CERTIV_APPLICATION_FORM, 'project_management_certiv_form_submission_process', 10, 2 );
...
function project_management_certiv_application_form_pdf($form) {
	// Start
	$content = neca_pdf_content_start ();
	
	// Add Heading
	$content .= neca_pdf_content_heading ( '2022 Application Form<br/>BSB40920 Cert. IV in Project Management Process', $form );
	...
	// Office Use Only
	$content .= neca_pdf_content_office_use_only_project_management_certiv_2020( $form );
	
	// Sign Off
	$content .= neca_pdf_content_sign_off_project_management_certiv_2022 ();
...
function neca_pdf_content_office_use_only_project_management_certiv_2020($form) {
...
function neca_pdf_content_sign_off_project_management_certiv_2020() {
...
function neca_pdf_content_sign_off_project_management_certiv_2022() {
	$content = '<div class="heading3">SIGN OFF</div>';
	...
	$content .= '	<tr>
					...
					<td style="width: 95%;">
						...
						<p class="small">NCVER may also disclose personal information to persons engaged by NCVER to conduct research on NCVER\'s behalf.</p>
						<p class="small">
							RTO Declaration and Understanding<br/>
							I declare that the information provided in this data submission is accurate and complete.<br/>
							...
							<ul>
								<li> information provided in this data submission will only be used, accessed, published and disseminated according to the National VET Data Policy</li>
								<li> if that information also includes personal information, the Privacy Act 1988, the Australian Privacy Principles and the National Vocational Education and Training Regulator Act 2011 regulate the collection, use or disclosure of personal information.</li>
							</ul>
							I understand that:<br/>
							<ul>
								<li>information provided in this data submission may be used for the purposes outlined above, and</li>
