...
// IOT Form (IOT-APP)
add_filter("gform_pre_render_" . IOT_FORM_ID, 'iot_form_prepopulate');
add_filter("gform_pre_validation_" . IOT_FORM_ID, 'iot_form_prepopulate');
add_filter("gform_pre_submission_filter_" . IOT_FORM_ID, 'iot_form_prepopulate');
add_filter("gform_admin_pre_render" . IOT_FORM_ID, 'iot_form_prepopulate');

function iot_form_prepopulate($form)
{
 ...
 * *************** */
// IOT Form (IOT-APP)
add_action( "gform_pre_submission_" . IOT_FORM_ID, 'iot_form_presubmission');

function iot_form_presubmission()
{
	if(IOT_DEBUG_MODE)
	{
...
/**
 * IOT FORM SUBMISSION PROCESS
 *
 * @param array $entry
 * @param object $form_data
 * @return boolean
 */
function iot_form_submission_process($form_data, $order, $item_cost) 
{
	if (IOT_DEBUG_MODE) 
	{
		echo "<h3>Internet of Things Submission Process</h3>";
		error_log ( "--- Internet of Things Submission Process ---" );
	}
	
	$form = new JobReadyForm ();
	
	// Gravity Form
	$form->gform_id = $form_data->{'id'};
	$form->gform_form_id = $form_data->{'form_id'};
	
	// Course Details
	$form->course_scope_code = $form_data->{'81'};
	$form->course_number = $form_data->{'192'};
	$form->invoice_option = $form_data->{'190'};
	$form->cost = $item_cost;
	
	// Personal Details
	$form->gender = $form_data->{'69'};
	$form->title = $form_data->{'2'};
	$form->first_name = ucwords ( strtolower ( $form_data->{'9'} ) );
	$form->middle_name = ucwords ( strtolower ( $form_data->{'83'} ) );
	...
	// Address
	$form->street_address1 = ucwords ( strtolower ( $form_data->{'97'} ) );
	$form->suburb = ucwords ( strtolower ( $form_data->{'98'} ) );
	$form->state = ucwords ( strtolower ( $form_data->{'99'} ) );
	$form->postcode = $form_data->{'100'};
	
	$form->postal_address_same = $form_data->{'87.1'}; // 87.1 because its a checkbox
	
	if ($form->postal_address_same != 'Yes') 
	{
		$form->postal_street_address1 = ucwords ( strtolower ( $form_data->{'101'} ) );
		$form->postal_suburb = ucwords ( strtolower ( $form_data->{'102'} ) );
		$form->postal_state = ucwords ( strtolower ( $form_data->{'103'} ) );
		$form->postal_postcode = $form_data->{'104'};
	}
	
	// Emergency Contact
	$form->emergency_contact_firstname = ucwords ( strtolower ( $form_data->{'71'} ) );
	$form->emergency_contact_surname = ucwords ( strtolower ( $form_data->{'92'} ) );
	$form->emergency_contact_number = $form_data->{'73'};
	$form->emergency_contact_email = $form_data->{'133'};
	$form->emergency_contact_relationship = ucwords ( strtolower ( $form_data->{'74'} ) );
	...
	$form->referred = $form_data->{'172'};
	if ($form->referred == 'Yes')
	{
		$form->referred_details = $form_data->{'173'};
	}
	
	// Country of Birth
	$form->country_of_birth = $form_data->{'93'};
	
	// Birth + Nationality + Indigenous + Language
	$form->australian_citizen = $form_data->{'187'};
	...
	{
		$form->citizenship_other = $form_data->{'188'};
	}
	...
	{
		$form->main_language = $form_data->{'95'};
		// $form->spoken_english_proficiency= "Very Well";
	}
	...
	/* 2021.03.16 - Disabled
	 ...
	 $form->school = ucwords ( strtolower ( $form_data->{'46'} ) );
	 }
	 */
	...
	// 2021.03.16 - Disabled
	//$form->year_highest_school_level = $form_data->{'105'};
	...
	if ($form->disability_flag == 'Yes') 
	{
		...
		{
			$ref = '43.' . $i;
			if (isset ( $form_data->{$ref} ) && $form_data->{$ref} != '') 
			{
				$form->disability_types [] = $form_data->{$ref};
			}
		}
		if ($form->disabilities_other = $form_data->{'124'} != '')
		{
			$form->disabilities_other = ucwords ( strtolower ( $form_data->{'124'} ) );
		}
	}
	...
	if ($form->prior_education_flag == 'Yes') 
	{
		...
		$form->prior_education_qualification = $form_data->{'52'};
	}
	
	// Employer Details
	$form->employer_party_id = $form_data->{'126'};
	$form->employer_search_or_create = $form_data->{'136'};
	...
	{
		$form->employer_company = ucwords ( strtolower ( $form_data->{'107'} ) );
		$form->employer_address = ucwords ( strtolower ( $form_data->{'108'} ) );
		$form->employer_suburb = ucwords ( strtolower ( $form_data->{'109'} ) );
		$form->employer_state = ucwords ( strtolower ( $form_data->{'110'} ) );
		$form->employer_postcode = $form_data->{'111'};
		$form->employer_office_phone = $form_data->{'112'};
	}
	$form->employer_paying_invoice = $form_data->{'118'};
	...
	$form->industry_employment = $form_data->{'96'};
	$form->occupation = $form_data->{'66'};
	
	// Concession Card
	//$form->concession_flag = $form_data->{'119'};
	
	// How did you hear
	$form->how_did_you_hear = ucwords ( strtolower ( $form_data->{'67'} ) );
	
	// Victorian Student
	$form->previous_victorian_education = $form_data->{'57'};
	if (strpos ( $form->previous_victorian_education, 'Yes' ) !== false) 
	{
		$form->vsn = $form_data->{'56'};
	}
	
	if (strpos ( $form->previous_victorian_education, 'Yes' ) !== false && isset ( $form_data->{'58'} ) && $form_data->{'58'} != '') 
	{
		$form->previous_victorian_school = ucwords ( strtolower ( $form_data->{'58'} ) );
	}
	if (strpos ( $form->previous_victorian_education, 'Yes' ) !== false && isset ( $form_data->{'59'} ) && $form_data->{'59'} != '') 
	{
		$form->previous_victorian_school = ucwords ( strtolower ( $form_data->{'59'} ) );
	}
	
	// 2020.03.04 - Required for Skills First Program PDF
	/* 2023.01.16 - Remove request by Ashima Nakra (email)
	$form->highest_qualification_completed = ucwords ( strtolower ( $form_data->{'146'} ) );
	$form->government_funded_enrolments_this_year = $form_data->{'148'};
	$form->government_funded_undertakings_at_present = $form_data->{'149'};
	$form->government_funded_in_lifetime = $form_data->{'150'};
	$form->enrolled_in_a_school = $form_data->{'152'};
	$form->enrolled_in_skills_for_education = $form_data->{'153'};
	$form->subsidized_acknowledgement = $form_data->{'154'};
	$form->contacted_by_department_acknowledgement = $form_data->{'155'};
	*/
	
	// 25.03.2021 - Credit Transfer + RPL
	$form->credit_transfer = $form_data->{'184'};
	$form->rpl = $form_data->{'186'};
	
	// 2018.09.05 - Signature
	$form->signature = ucwords ( strtolower ( $form_data->{'170'} ) );
	
	// Student Declaraction
	$form->privacy_declaration = $form_data->{'60.1'};
	...
	$iotfp = iot_application_form_pdf ( $form );
	
	if(IOT_DEBUG_MODE)
	{
		echo '<a href="' . JR_ROOT_URL . '/pdf/' . $iotfp . '" target="_blank">Internet of Things - Applcation Form (PDF)</a><br/><br/>';
	}
	
	// Create Skills First PDF
	/* 2023.01.16 - Remove request by Ashima Nakra (email)
	$course_name = 'IOT Application Form';
	...
	*/
	...
	if (isset ( $party_id ) && isset ( $employer_party_id )) 
	{
		...
		*/
		...
		$pdf_file = JR_ROOT_PATH . '/pdf/' . $iotfp;
		$document = new JRAPartyDocument ();
		$document->party_id = $party_id;
		$document->name = 'Internet of Things Application Form (' . $form->course_scope_code . ' - ' . $form->course_number . ')';
		$document->description = 'Internet of Things Application Form (' . $form->course_scope_code . ' - ' . $form->course_number . ')';
...
// IOT Application Form PDF
function iot_application_form_pdf($form) {
	// Start
	$content = neca_pdf_content_start ();
	
	// Add Heading
	$content .= neca_pdf_content_heading ( '2023 Application Form<br/>22585VIC Internet of Things', $form );
	...
	// Office Use Only
	$content .= neca_pdf_content_office_use_only_iot_2023 ( $form );
	
	// Sign Off
	$content .= neca_pdf_content_sign_off_iot_2023 ();
...
function neca_pdf_content_office_use_only_iot_2023($form) {
	$content = '<div class="heading3">OFFICE USE ONLY</div>';
	
	$content .= '<table class="tbl">
					<tr>
						<td colspan="2">
							<strong>PROGRAM DETAILS (Internet of Things IOT)</strong><br/>
...
function neca_pdf_content_sign_off_iot_2023() {
