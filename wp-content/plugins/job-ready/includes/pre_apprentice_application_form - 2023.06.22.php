...
function pre_apprentice_application_form_prepopulate($form)
{
	...
	if(isset($_SESSION['prefill']))
	{
		...
		*/
		...
		$prefill_fields['211'] = isset($_SESSION['prefill']->credit_transfer) ? $_SESSION['prefill']->credit_transfer : '';
		$prefill_fields['169'] = isset($_SESSION['prefill']->rpl) ? $_SESSION['prefill']->rpl : '';
...
function non_apprentice_application_form_presubmission()
{
	...
	*/
	...
	if(PREAPPRENTICE_DEBUG_MODE)
	{
		echo "SESSION Prefile Variable: <br/>";
...
function pre_apprentice_application_form_submission_process($entry, $form_data) 
{
	if (PREAPPRENTICE_DEBUG_MODE) 
	{
		echo "<h3>Pre-Apprentice Application Form - Submission Process</h3>";
// 		echo "Raw Form Data: <br/>";
// 		var_dump ( $form_data );
// 		echo "<br/><br/>";
	}
	...
	//$form->invoice_option = rgar($entry, '98');
	//$form->cost = $item_cost;
	...
	$form->credit_transfer = rgar($entry, '211');
	$form->rpl = rgar($entry, '169');
	...
	$form->prerequisite_declaration = isset ($entry['121.1']) ? 'Yes' : '';
	$form->privacy_declaration = isset ($entry['130.1']) ? 'Yes' : '';
	...
	*/
	...
	if (PREAPPRENTICE_DEBUG_MODE)
	{
		echo '<a href="' . JR_ROOT_URL . '/pdf/' . $naafp. '" target="_blank">Pre-Apprentice Applcation Form (PDF)</a><br/><br/>';
	}
	...
	*/
	...
	if ($count > 0) 
	{
		...
		else 
		{
			if (PREAPPRENTICE_DEBUG_MODE) 
			{
				echo "An error occurred while updating a Party Resource<br/><br/>";
			}
			return false;
		}
	}
	else
	{
		...
		else 
		{
			if (PREAPPRENTICE_DEBUG_MODE) 
			{
				echo "An error occurred while creating a Party Resource<br/><br/>";
			}
			return false;
		}
	}
	...
	if (isset ( $party_id )) 
	{
		...
		if (PREAPPRENTICE_DEBUG_MODE) 
		{
			echo "Check if Party Contact Exists<br/>";
		}
	...
	else
	{
		if (PREAPPRENTICE_DEBUG_MODE) 
		{
			echo "No Party ID available<br/><br/>";
		}
	}
}
